...
import { BestPredictionResultData, BestPredictionResultImage } from "./vis/BestPredictionResultImage"
...
function init(base: D3Sel) {
    const html = `
        <div id="main-popup" class="hidden"></div>
    ...
    const eventHelpers = {
        updatePage: () => {
            ...
            api.getSaliencyImage(state.caseStudy(), state.selectedImage(), state.scoreFn()).then(img => {
                ...
                const maskData = vizs.interactiveSaliencyMask.getDrawCanvas(224,224).toDataURL().slice(22) // Remove data/png info
                ...
                api.getBestPrediction(state.selectedImage(), maskData, state.scoreFn(), topk).then(r => {
                    ...
                    const resultMasks = sels.resultsPanel.selectAll('.result-mask-image')
                        .data(r)
                        .join('div')
